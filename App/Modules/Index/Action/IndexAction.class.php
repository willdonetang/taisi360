...
{ 
	Public function index(){
		...
		$article=M('article');
		//首页中部3篇文章推荐展示
		$mPushArticle=$article->where('isslides=1 and islock=0')->order('rand()')->limit(5)->select();
		//首页下部4个分类推荐展示
		$dPushCats = $cats->limit('4')->where('ispush=1 and isverify=1')->relation(false)->select();
        foreach($dPushCats as $key=>&$cat){
            $tid = $cat['id'];
            $articles = $article->where('islock=0 and tid='.$tid)->order('pubtime desc')->limit(10)->select();

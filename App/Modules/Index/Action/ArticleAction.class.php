...
class ArticleAction extends CommonAction
{
	...
	public function addComment(){
		if($_SESSION[C('USER_AUTH_KEY_ID')]){
            if($_SESSION["comment_verify"]!=md5($_POST['code'])){
                echo "验证码错误";
                die;
            }
			//对回复速度进行判断
			...
			$is_dirty = $this->check_dirty($comment);
			if($is_dirty == true){
				echo '系统出错，请稍候再评论';
				die;
			}
    ...
    //检查是否命中屏蔽词，命中则返回true，不命中则返回 false
    private function check_dirty($comment){
        $ret = false;
        $dirty_pattern = array("/fuzokuu/i","/盜.*用/","/淫.*狐/","/盜.*文/","/風.*俗/");
        foreach($dirty_pattern as $word){
    ...
    public function get_qrcode(){
        $content = I('content');
        include(APP_PATH.'/Lib/QRCode/phpqrcode.php');
